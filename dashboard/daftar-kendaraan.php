  <?php
  // dashboard/daftar-kendaraan.php — v2 CREAM EDITION
  session_start();
  ...
  <head>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    :root {
      ...
      --fg-low:         rgba(10,10,10,0.32);
      --border:         rgba(10,10,10,0.08);
      --border-acc:     rgba(83,104,120,0.25);

      /* Status */
      --green:          #488c60;
      --green-bg:       rgba(72,140,96,0.08);
      --green-bdr:      rgba(72,140,96,0.22);
      --red:            #b03a2e;
      --red-bg:         rgba(176,58,46,0.07);
      --red-bdr:        rgba(176,58,46,0.22);
    }
    ...
    .nav-inner {
      ...
      padding: 0 40px; height: 62px; max-width: 960px; margin: 0 auto;
    }
    .nav-brand { display: flex; align-items: center; gap: 12px; text-decoration: none; }
    .nav-brand-icon {
      width: 32px; height: 32px;
      border: 1px solid var(--border-acc); background: var(--accent-glow);
    ...
    .nav-back:hover { color: var(--accent); border-color: var(--border-acc); background: var(--accent-glow); }
    .nav-back i { font-size: 10px; }

    /* ── PAGE ────────────────────────────────────────────────── */
    .page { max-width: 760px; margin: 0 auto; padding: 52px 24px 100px; }
    .page-eyebrow { font-size: 9.5px; font-weight: 800; letter-spacing: 0.22em; text-transform: uppercase; color: var(--accent); margin-bottom: 12px; display: block; }
    ...
    .step-indicator::before {
      ...
      background: linear-gradient(90deg, var(--accent), rgba(83,104,120,0.3));
    }
    ...
    .step-num {
      ...
      display: flex; align-items: center; justify-content: center;
      font-size: 11px; font-weight: 800; color: var(--fg-low);
    ...
    .step-item.active .step-num   { border-color: var(--border-acc); background: var(--accent-glow); color: var(--accent); }
    ...
    .field-label {
      ...
      color: var(--fg-low); display: flex; align-items: center; gap: 6px; margin-bottom: 8px;
    }
    ...
    .field-icon {
      ...
      font-size: 12px; color: var(--fg-low); pointer-events: none;
    ...
    .field-input:focus { border-color: var(--border-acc); background: #fff; }
    ...
    .custom-select-trigger {
      ...
      user-select: none; transition: border-color 0.2s, background 0.2s; position: relative;
    }
    .custom-select-trigger:hover       { border-color: var(--border-acc); background: rgba(83,104,120,0.03); }
    .custom-select-trigger.open        { border-color: var(--border-acc); background: #fff; }
    ...
    .custom-select-panel {
      ...
      background: #fff; border: 1px solid var(--border-acc);
      z-index: 999; display: none; flex-direction: column;
      max-height: 280px; overflow: hidden;
      box-shadow: 0 12px 40px rgba(83,104,120,0.14);
    }
    ...
    .cs-search-wrap {
      padding: 10px; border-bottom: 1px solid var(--border); flex-shrink: 0; position: relative;
    }
    .cs-search {
    ...
    .cs-search:focus { border-color: var(--border-acc); background: #fff; }
    ...
    .cs-options::-webkit-scrollbar-thumb { background: var(--border-acc); border-radius: 2px; }
    ...
    .cs-option {
      ...
      cursor: pointer; display: flex; align-items: center; justify-content: space-between;
      transition: background 0.14s, color 0.14s; font-family: var(--font-sans);
    }
    .cs-option:hover    { background: var(--surface); color: var(--fg); }
    ...
    .upload-area {
      border: 1px dashed var(--border-acc); background: var(--accent-glow);
    ...
    .upload-area:hover,
    .upload-area.dragover { border-color: var(--accent); background: var(--accent-dim); }
    ...
    .upload-preview { position: absolute; inset: 0; background: var(--surface); display: none; align-items: center; justify-content: center; overflow: hidden; }
    ...
    .upload-preview-del  {
      ...
      display: flex; align-items: center; justify-content: center;
      color: var(--red); font-size: 9px; z-index: 2;
    }

    /* ── REVIEW ──────────────────────────────────────────────── */
    .review-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 0; border: 1px solid var(--border); }
    .review-item { padding: 14px 18px; border-bottom: 1px solid var(--border); border-right: 1px solid var(--border); background: var(--bg); }
    ...
    .btn-next:hover, .btn-submit:hover { background: var(--accent); color: #fff; transform: translateY(-2px); }
    ...
    .btn-dashboard:hover { background: var(--accent); color: #fff; text-decoration: none; }
            ...
            <div class="custom-select-wrap" id="cs-kelas-wrap">
              ...
              <div class="custom-select-panel" id="cs-kelas-panel" role="listbox">
                ...
                <div class="cs-options" id="cs-kelas-options">
                  ...
                  <?php endif; ?>
                  <div class="cs-option <?= $sel_kelas === $kl ? 'selected' : '' ?>" data-value="<?= $kl ?>" role="option">
                    <?= $kl ?>
                  </div>
                  <?php endforeach; ?>
                </div>
            ...
            <div class="custom-select-wrap" id="cs-jenis-wrap">
              ...
              <div class="custom-select-panel" id="cs-jenis-panel" role="listbox">
                <div class="cs-options" id="cs-jenis-options">
                  <div class="cs-option selected" data-value="motor" role="option">&nbsp; Motor</div>
  ...
  function initCustomSelect(cfg) {
    ...
    function open()   { trigger.classList.add('open'); trigger.setAttribute('aria-expanded','true'); panel.classList.add('open'); if (searchEl) { searchEl.value=''; filterOptions(''); searchEl.focus(); } }
    function close()  { trigger.classList.remove('open'); trigger.setAttribute('aria-expanded','false'); panel.classList.remove('open'); }
    ...
    document.addEventListener('click', function(e){ if(!trigger.closest('.custom-select-wrap').contains(e.target)) close(); });
    ...
    function filterOptions(q) {
      ...
      q = q.toLowerCase();
      opts.forEach(function(o){ o.classList.toggle('hidden', !o.dataset.value.toLowerCase().includes(q)); });
    ...
    optionsEl && optionsEl.addEventListener('click', function(e){
      ...
      trigger.classList.remove('placeholder','invalid'); trigger.classList.add('valid');
  ...
  initCustomSelect({ triggerId:'cs-kelas-trigger', textId:'cs-kelas-text', panelId:'cs-kelas-panel', optionsId:'cs-kelas-options', searchId:'cs-kelas-search', realSelectId:'kelas-real', msgId:'msg-kelas' });
  initCustomSelect({ triggerId:'cs-jenis-trigger', textId:'cs-jenis-text', panelId:'cs-jenis-panel', optionsId:'cs-jenis-options', realSelectId:'jenis-real', onSelect:function(val,text){ document.getElementById('cs-jenis-text').textContent=text; } });

  if (selectedKelas) {
    var initOpt = document.querySelector('#cs-kelas-options .cs-option[data-value="'+selectedKelas+'"]');
    if (initOpt) { document.getElementById('cs-kelas-text').textContent=selectedKelas; document.getElementById('cs-kelas-trigger').classList.remove('placeholder'); }
  }

  /* ── Step Navigation ─────────────────────────────────────── */
  function goNext(from) { if (!validateStep(from)) return; collectData(from); switchStep(from, from+1, false); }
  function goPrev(from) { switchStep(from, from-1, true); }
  window.goNext = goNext; window.goPrev = goPrev;

  function switchStep(from, to, reverse) {
    var pFrom=document.getElementById('panel-'+from), pTo=document.getElementById('panel-'+to);
    if (!pFrom||!pTo) return;
    ...
    currentStep=to; updateIndicator();
    if (to===3) buildReview();
  ...
  function updateIndicator() {
    document.querySelectorAll('.step-item').forEach(function(el){
      var s=+el.dataset.step; el.classList.remove('active','done');
      var snum=document.getElementById('snum-'+s);
      if (s<currentStep)      { el.classList.add('done');   snum.innerHTML='<i class="fa-solid fa-check" style="font-size:11px"></i>'; }
      else if (s===currentStep){ el.classList.add('active'); snum.textContent=s; }
      else                     { snum.textContent=s; }
    });
  }
  ...
  function validateStep(step) {
    ...
    if (step===1) {
      ...
      var kr=document.getElementById('kelas-real'), msg=document.getElementById('msg-kelas'), trig=document.getElementById('cs-kelas-trigger');
      if (!kr||!kr.value) { if(trig) trig.classList.add('invalid'); if(msg){ msg.className='field-msg show err'; msg.innerHTML='<i class="fa-solid fa-circle-exclamation"></i> Pilih kelas terlebih dahulu'; } ok=false; }
  ...
  function vf(id, rule) {
    var el=document.getElementById(id), msg=document.getElementById('msg-'+id);
    if (!el) return true;
    var err=rule(el.value.trim());
    if (err) { el.classList.add('invalid'); el.classList.remove('valid'); if(msg){msg.className='field-msg show err';msg.innerHTML='<i class="fa-solid fa-circle-exclamation"></i> '+err;} el.focus(); return false; }
  ...
  Object.keys(RULES).forEach(function(id){
    ...
    el.addEventListener('input', function(){ el.classList.remove('invalid','valid'); var m=document.getElementById('msg-'+id); if(m){m.className='field-msg';m.innerHTML='';} });
    el.addEventListener('blur',  function(){ if(!el.value.trim()) return; vf(id, RULES[id]); });
  });
  ...
  function collectData(step) {
    if (step===1) {
      ['nama_lengkap','nis','umur','email','no_telepon'].forEach(function(id){ var el=document.getElementById(id); if(el) formData[id]=el.value; });
      formData['kelas']=document.getElementById('kelas-real').value;
    }
    if (step===2) {
      ['nama_kendaraan','nomor_tnkb'].forEach(function(id){ var el=document.getElementById(id); if(el) formData[id]=el.value; });
      formData['jenis']=document.getElementById('jenis-real').value;
    }
  }
  function buildReview() {
    var box=document.getElementById('review-summary'); if(!box) return;
  ...
  ['kendaraan','sim','kartu'].forEach(function(key){
    var inp=document.getElementById('file-'+key), area=document.getElementById('area-'+key);
    var prev=document.getElementById('prev-'+key), img=document.getElementById('previmg-'+key);
    if (!inp) return;
    inp.addEventListener('change', function(){ showPreview(this,prev,img,key); });
    area.addEventListener('dragover',  function(e){ e.preventDefault(); area.classList.add('dragover'); });
    area.addEventListener('dragleave', function(){ area.classList.remove('dragover'); });
    area.addEventListener('drop', function(e){ e.preventDefault(); area.classList.remove('dragover'); if(e.dataTransfer.files.length){ inp.files=e.dataTransfer.files; showPreview(inp,prev,img,key); } });
  });

  function showPreview(inp,prev,img,key) {
    var file=inp.files[0]; if(!file) return;
    var msgEl=document.getElementById('msg-foto_'+key);
    if (file.size>2*1024*1024) { if(msgEl){msgEl.className='field-msg show err';msgEl.innerHTML='<i class="fa-solid fa-circle-exclamation"></i> Ukuran maks. 2MB';} inp.value=''; return; }
  ...
  function clearFile(key) {
    var inp=document.getElementById('file-'+key), prev=document.getElementById('prev-'+key);
    var img=document.getElementById('previmg-'+key), msg=document.getElementById('msg-foto_'+key);
    if(inp) inp.value=''; if(prev) prev.classList.remove('show'); if(img) img.src='';
